...
def load_data(file_csv):
    ...
    data_bq = pd.read_csv(file_csv)

    # Reformatage des données
    data_bq['Amount'] = data_bq['Amount'].str.replace(',', '.', regex=False).astype(float)
    data_bq['Started Date'] = pd.to_datetime(data_bq['Started Date'])
    data_bq['year'] = data_bq['Started Date'].dt.year
    data_bq['day'] = data_bq['Started Date'].dt.day_name(locale='fr_FR') + ' ' + data_bq['Started Date'].dt.day.astype(str)
    data_bq['day_id'] = data_bq['Started Date'].dt.day
    data_bq['month'] = data_bq['Started Date'].dt.month_name(locale='fr_FR')
    data_bq['month_id'] = data_bq['Started Date'].dt.month


    data_bq = data_bq[['Description', 'Amount', 'day','day_id', 'month', 'month_id', 'year', 'Type']]
    new_labels = ['label', 'amount', 'day','day_id', 'month', 'month_id', 'year', 'type']

    for i, new_label in enumerate(new_labels):
        old_label = data_bq.columns.values[i]
        if old_label!=new_label:
            data_bq.rename(columns={old_label : new_label}, inplace=True)
...
def get_list(data_bq, label1, label2=None):
    
    if label2:
        # Concatène deux Series puis filtre les doublons et NaN
        data_bq['label_concat']= data_bq[label1]+str(data_bq[label2])
        return data_bq
    

    return [''] + data_bq[label1].dropna().unique().tolist()




def get_day(data_bq, label):
    return


fichier_csv = ('csv/revolut_releve.csv')
...
'''
'''mask_cafe = data_bq['Description']=='DIS\'Automatic'
mask_papaia = data_bq['Description']=='Papaia'

 


sum_per_desc = data_bq.groupby(['Description'])['Amount'].sum().reset_index()
sum_per_desc_date = data_bq.groupby(['year', 'Mois', 'Description'])['Amount'].sum().reset_index()
