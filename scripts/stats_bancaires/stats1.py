...
def stats1():
    if 'data' in st.session_state:
        ...
        st.subheader("Dépenses par enseigne")

        data_label_picked = data.copy()

        ### Menu déroulant - label ###
        data_label = get_list(data, 'label')
        label = st.selectbox("Enseigne", data_label, index=0)

        if label:
            data_label_picked = data[data['label'] == label]
        ...
        if st.session_state.year:
            ...
            data_label_picked.to_csv('res2.csv')
